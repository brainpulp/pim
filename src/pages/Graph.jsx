        ...
        return (
          <YTFullscreenPlayer clips={clips} startIndex={start} sound={yn?.ytss?.sound !== false} transition={yn?.ytss?.transition || 'fade'} fadeMs={yn?.ytss?.fadeMs ?? 1000} presenting={isPresenting}
            onExit={() => {
              ...
              // onExit now fires only on Esc (arrows past-end use onDeckNext). Esc during a presentation
              // ends the show — matching the deck's global Esc — instead of dumping to the canvas node.
              if (presentingSlideIdxRef.current !== null) { exitPresentation(); return }
